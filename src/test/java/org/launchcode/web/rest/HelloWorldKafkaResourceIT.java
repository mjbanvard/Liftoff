...
import org.launchcode.config.KafkaProperties;
...
class HelloWorldKafkaResourceIT {
    ...
    void setup() {
        ...
        HelloWorldKafkaResource kafkaResource = new HelloWorldKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/hello-world-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/hello-world-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
